...
describe('MaterialController (e2e)', () => {
  ...
  beforeEach(async () => {
    svc = {
      ...
      filterByType: jest.fn().mockResolvedValue({ data: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } }),
      findById: jest.fn().mockResolvedValue({ _id: 'mat-1', material_id: 'MAT-1' }),
  ...
  it('GET /materials forwards paging to service', async () => {
    await request(app.getHttpServer()).get('/materials?page=1&limit=10').expect(200);
  ...
  it('POST /materials creates a material', async () => {
    ...
    const res = await request(app.getHttpServer()).post('/materials').send(payload).expect(201);
