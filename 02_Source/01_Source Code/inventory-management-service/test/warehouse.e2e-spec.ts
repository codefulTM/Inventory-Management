...
describe('WarehouseController (e2e)', () => {
  ...
  beforeEach(async () => {
    svc = {
      ...
      search: jest.fn().mockResolvedValue({ data: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } }),
      getOptions: jest.fn().mockResolvedValue({ items: [], total: 0 }),
      findById: jest.fn().mockResolvedValue({ _id: '1', warehouse_id: 'WH-1' }),
      create: jest.fn().mockImplementation((dto) => ({ ...dto, _id: 'created' })),
  ...
  it('GET /warehouses forwards paging to service', async () => {
    const res = await request(app.getHttpServer()).get('/warehouses?page=2&limit=5').expect(200);
  ...
  it('POST /warehouses creates a warehouse', async () => {
    ...
    const res = await request(app.getHttpServer()).post('/warehouses').send(payload).expect(201);
    expect(svc.create).toHaveBeenCalledWith(expect.objectContaining({ warehouse_id: 'WH-NEW' }));
    expect(res.body).toHaveProperty('_id', 'created');
  });

  it('GET /warehouses/:id returns warehouse', async () => {
    const res = await request(app.getHttpServer()).get('/warehouses/1').expect(200);
