...
describe('BinWorklistController (e2e)', () => {
  ...
  it('GET /bins/worklist forwards params', async () => {
    await request(app.getHttpServer()).get('/bins/worklist?warehouse_id=WH-1&q=abc&page=1&limit=20').expect(200);
  ...
  it('GET /bins/:bin_code and counts call service', async () => {
    ...
    await request(app.getHttpServer()).get('/bins/BIN-01/counts?page=2&limit=5').expect(200);
    expect(svc.getBinCounts).toHaveBeenCalledWith('BIN-01', 2, 5);
  });

  it('POST /bins/:bin_code/counts submits counts', async () => {
    await request(app.getHttpServer()).post('/bins/BIN-01/counts').send({ counts: [{ material_id: 'MAT-1', quantity: 3 }] }).expect(200);
    expect(svc.submitCounts).toHaveBeenCalled();
  });

  it('POST/PUT/DELETE bin endpoints call service', async () => {
    await request(app.getHttpServer()).post('/bins').send({ bin_code: 'BIN-NEW' }).expect(201);
    expect(svc.createBin).toHaveBeenCalled();

    await request(app.getHttpServer()).put('/bins/BIN-NEW').send({ expected_qty: 5 }).expect(200);
