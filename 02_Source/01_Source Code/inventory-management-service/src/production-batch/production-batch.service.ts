...
/**
 * Valid status transitions for production batch (state machine)
 * - InProgress → Complete, OnHold, Cancelled
 * - OnHold → InProgress, Cancelled
 * - Complete → Terminal state (no further changes)
...
/**
 * ProductionBatchService - Service xử lý nghiệp vụ lô sản xuất
 * 
 * Chức năng chính:
 * - Tạo mới lô sản xuất (tự động sinh batch_id)
 * - Quản lý vòng đời lô: OnHold → InProgress → Complete/Cancelled
 * - Khi hoàn thành (Complete): 
 *   + Kiểm tra tồn kho các thành phần
 *   + Tự động trừ kho nguyên liệu
 *   + Tạo lô thành phẩm (inventory lot) với trạng thái Quarantine
 * - Tính toán hạn sử dụng từ shelf_life của thành phẩm
 * - Quản lý thành phần (batch components)
 * 
 * Quy tắc nghiệp vụ:
 * - batch_number phải duy nhất
 * - product_id phải tồn tại trong Materials
 * - shelf_life_value > 0, shelf_life_unit phải là day/month/year
 * - Chỉ được xóa khi không ở trạng thái InProgress
 * - Khi Complete: Tất cả thành phần phải có tồn kho đủ
 */
@Injectable()
export class ProductionBatchService {
  ...
  /**
   * Helper: Tính toán expiration_date từ shelf_life
   * 
   * @param receivedDate - Ngày nhận (ngày sản xuất xong)
   * @param shelfLifeValue - Giá trị shelf life
   * @param shelfLifeUnit - Đơn vị (day/month/year)
  ...
  /**
   * Bước 1: Kiểm tra tồn kho các thành phần có đủ không
   * 
   * @param components - Danh sách thành phần với lot_id và planned_quantity
   * @returns true nếu tất cả đều đủ tồn kho
   * @throws BadRequestException nếu thiếu tồn kho
   * @throws NotFoundException nếu không tìm thấy lot
   */
  ...
  /**
   * Bước 2: Trừ kho nguyên liệu và tạo giao dịch
   * 
   * @param batchId - ID lô sản xuất
   * @param components - Các thành phần cần trừ kho
   * @param performedBy - Người thực hiện
   */
  ...
  /**
   * Bước 3: Tạo lô thành phẩm (inventory lot) sau khi sản xuất xong
   * 
   * @param batch - Lô sản xuất (đã hoàn thành)
   * @param performedBy - Người thực hiện
   * @returns Lô thành phẩm vừa tạo
   */
  async createFinishedProductLot(
    ...
    const createdLot = await this.inventoryLotService.create({
      ...
      quantity: Number(batch.batch_size),
    ...
    return createdLot as any;
  }

  /**
   * Tạo mới lô sản xuất
   * 
   * @param createDto - Dữ liệu tạo lô sản xuất
   * @returns ProductionBatchResponseDto - Thông tin lô vừa tạo
   * @throws ConflictException nếu batch_number đã tồn tại
   * @throws NotFoundException nếu product_id không tồn tại
   * @throws BadRequestException nếu shelf_life không hợp lệ
   */
  async create(
    ...
    const batchToCreate = {
      ...createDto,
      batch_id: createDto.batch_id || createDto.batch_number,
  ...
  /**
   * Lấy tất cả lô sản xuất có phân trang
   */
  async findAll(
    page: number = 1,
    limit: number = 20,
  ...
  /**
   * Lấy một lô sản xuất theo ID
   * Tìm được cả theo _id và batch_id
   */
  ...
  /**
   * Lấy các lô sản xuất theo product_id
   */
  ...
  /**
   * Lấy các lô sản xuất theo trạng thái
   */
  async findByStatus(
    status: string,
    page: number = 1,
   ...
   * Tự động xử lý quy trình hoàn thành (Complete)
   * 
   * @param batchId - batch_id hoặc _id
    ...
    if (updateDto.status && updateDto.status !== existing.status) {
      ...
      if (updateDto.status === BatchStatus.Complete) {
        try {
          ...
          // Bước 2: Verify inventory availability
          await this.verifyInventoryAvailability(components);

          // Bước 3: Deduct materials from inventory and create transactions
          ...
          // Bước 4: Create finished product lot
  ...
  /**
   * Xóa lô sản xuất
   * Không cho phép xóa khi đang InProgress
   */
  ...
  /**
   * Chuyển đổi từ ProductionBatch Document sang ProductionBatchResponseDto
   */
