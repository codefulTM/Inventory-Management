// Inventory Analyst Agent - Agent phân tích tồn kho và hạn sử dụng
// Chức năng: Thống kê tồn kho, cảnh báo lô sắp hết hạn/đã hết hạn, truy vấn RAG
// Sử dụng Gemini để sinh phản hồi tự nhiên cho người dùng cuối
...
// Định nghĩa các vai trò người dùng để cá nhân hóa phản hồi
type UserRole =
  | "manager" // Quản lý: Cần tổng quan và quyết định chiến lược
  | "operator" // Vận hành: Cần hướng dẫn thao tác FIFO, xuất nhập
  | "quality-control" // QC: Cần cảnh báo điều kiện bảo quản
  | "it_admin" // IT Admin: Quản trị hệ thống
  | "unknown"; // Chưa xác định

// Thông tin lô hàng gần hết hạn nhất
type NearestExpiryLot = {
  sourceId: string; // ID của lô
  expirationDate: string; // Ngày hết hạn
  daysRemaining: number; // Số ngày còn lại
};

// Thông tin tóm tắt giao dịch kho
type TransactionDigest = {
  id: string; // ID giao dịch
  type: string; // Loại giao dịch (IN/OUT)
  materialId: string; // ID vật tư/lô
  quantity: string; // Số lượng + đơn vị
  happenedAt: string; // Thời điểm giao dịch
};

@Injectable()
export class InventoryAnalystAgent {
  // Hồ sơ của agent (profile) - Được gửi cho LLM để định hướng phản hồi
  private readonly profile = {
    ...
    tools: [
      // Danh sách các công cụ agent có thể sử dụng
  ...
  constructor(
    // Service gọi backend qua gRPC để lấy dữ liệu tồn kho
    ...
    // Service tạo embedding vector cho hybrid search
    private readonly queryEmbeddingService: QueryEmbeddingService,
  ) {}

  // Phương thức chính xử lý yêu cầu phân tích tồn kho
  // Quy trình: Kiểm tra domain -> Lấy dữ liệu -> RAG search -> Sinh phản hồi
  async handle(input: AgentHandlerInput): Promise<AgentHandlerOutput> {
    try {
      // Kiểm tra xem query có thuộc miền tồn kho không
      ...
      // Chuẩn hóa query để so khớp từ khóa
      const normalizedQuery = this.normalizeForMatching(input.query || "");

      // Đọc tham số phân trang và vai trò người dùng từ payload
      ...
      // Phát hiện ý định: Xem giao dịch gần đây?
      ...
      // Lấy đồng thời: Thống kê lô hàng và Danh sách giao dịch
      ...
      // Trích xuất: Số ngày cảnh báo, ý định hết hạn/hết hạn
      ...
      // Khởi tạo dữ liệu lô và RAG retrieval
      ...
      // Định nghĩa các collection để tìm kiếm
      const businessCollections = [
        "inventory_lots", // Lô hàng
        "inventory_transactions", // Giao dịch kho
        "qc_tests", // QC tests
      ];
      const docsCollections = ["docs_knowledge"]; // Tài liệu hướng dẫn

      // Lấy danh sách lô sắp hết hạn/hết hạn nếu người dùng hỏi
      if (asksExpiringSoon)
        expiringLots =
      ...
      try {
        // Bước 1: Tạo embedding vector từ query
        const queryEmbedding = await this.queryEmbeddingService.embedQuery(
          input.query,
        );

        // Bước 2: Thử hybrid search (keyword + semantic) nếu có embedding
        if (queryEmbedding && queryEmbedding.length > 0) {
          try {
            ...
            // Fallback sang semantic search nếu hybrid thất bại
          ...
          // Nếu không có embedding thì dùng semantic search
          retrieval = await this.backendDataService.semanticSearch(
            input.query,
            5,
            businessCollections,
        ...
        // Bước 3: Nếu không có kết quả từ business data, thử tìm trong docs
      ...
      // Tạo highlights và citations từ kết quả RAG
      ...
      // Trích xuất lô gần hết hạn nhất từ citations
      ...
      // Tạo insights (thông tin chi tiết) dựa trên dữ liệu
      ...
      // Tổng hợp dữ liệu ngữ cảnh để gửi cho LLM
      const contextData = {
        lots: lotStats,
        expiringLots,
        expiredLots,
      ...
      // Làm sạch phản hồi và kiểm tra xem có phù hợp với query không
      ...
      // Sử dụng phản hồi từ LLM hoặc fallback template
  ...
  // Tạo phản hồi dự phòng (fallback) khi LLM không phản hồi phù hợp
  // Dựa trên dữ liệu thống kê và vai trò người dùng
  private buildFallbackReply(
    ...
    // Tạo lời khuyên dựa trên vai trò người dùng
    ...
    // Nếu có insights (thông tin chi tiết) thì ưu tiên hiển thị
    if (insights.length > 0) {
      return `${insights.join(" ")} ${roleGuidance}`.trim();
    }

    // Xử lý yêu cầu xem giao dịch gần đây
    ...
    // Phản hồi tổng quan tồn kho (không hỏi cụ thể về hạn)
    if (!asksExpiringSoon && !asksExpired) {
      ...
      // Thêm thông tin lô gần hết hạn nhất nếu có
      if (nearestExpiryLot) {
        summaryParts.push(
    ...
    // Phản hồi khi hỏi về lô sắp hết hạn
    if (asksExpiringSoon && !asksExpired) {
    ...
    // Phản hồi khi hỏi về lô đã hết hạn
  ...
  // Chuẩn hóa vai trò người dùng từ payload
  // Chuyển đổi từ display name sang role slug
  private normalizeUserRole(inputRole: unknown): UserRole {
    if (typeof inputRole !== "string") return "unknown";

    // Mapping từ tên hiển thị sang role slug
  ...
  // Chuyển đổi an toàn sang số, mặc định là 0 nếu không hợp lệ
  private toSafeNumber(value: unknown): number {
    const parsed = Number(value);
    return Number.isFinite(parsed) && parsed >= 0 ? parsed : 0;
  }

  // Tạo lời khuyên dựa trên vai trò người dùng và tình trạng rủi ro
  private buildRoleGuidance(
    ...
    // Lời khuyên cho Vận hành (Operator)
    ...
    // Lời khuyên cho QC (Quality Control)
    ...
    // Lời khuyên mặc định cho người dùng chưa đăng nhập/unknown
    return hasRiskLots
  ...
  // Làm sạch phản hồi từ LLM: Loại bỏ các từ khóa kỹ thuật (RAG, embedding...)
  private sanitizeAssistantReply(reply?: string): string {
    if (!reply) return "";

    // Tách các dòng và lọc bỏ dòng rỗng
    const lines = reply
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean);

    // Danh sách từ khóa cần lọc bỏ (thuật ngữ kỹ thuật)
    const blockedTokens = [
      "truy xuất",
      "tài liệu",
      "retrieval",
    ...
    // Lọc bỏ các dòng chứa từ khóa bị chặn
  ...
  // Kiểm tra xem phản hồi có phù hợp với ý định của người dùng không
  private isReplyAlignedToQuery(
    ...
    // Các tín hiệu nhận biết phản hồi về lô sắp hết hạn
    ...
    // Các tín hiệu nhận biết phản hồi về lô đã hết hạn
    const hasExpiredSignal =
      ...
      normalized.includes("het hsd");

    // Các tín hiệu nhận biết phản hồi về giao dịch
    ...
    // Kiểm tra sự khớp giữa ý định và phản hồi
    if (asksRecentTransactions) {
    ...
    // Kiểm tra phản hồi tổng quan tồn kho
    return (
  ...
  private isInventoryDomainQuery(query: string, action?: string): boolean {
    ...
    // Nếu action là inventory hoặc report thì chấp nhận luôn
    if (
    ...
    // Danh sách từ khóa miền tồn kho
    const keywords = [
      "het han", "sap het han", "con han", "can han", "can date",
      "han dung", "near expiry", "near-expiry", "expired", "expiring",
      "het date", "qua date", "bao cao", "ton kho", "stock",
      "inventory", "report", "lot", "batch", "fifo", "transaction",
  ...
  private detectRecentTransactionsIntent(normalizedQuery: string): boolean {
    const keywords = [
      "transaction", "transactions", "giao dich", "xuat nhap",
      "lich su kho", "recent", "gan day", "moi nhat", "latest",
    ];

    return this.containsAny(normalizedQuery, keywords);
  }

  // Trích xuất giới hạn số lượng giao dịch từ query
  private extractTransactionLimit(normalizedQuery: string): number {
    const matched = normalizedQuery.match(/\b(\d{1,3})\b/);
    if (!matched?.[1]) return 10; // Mặc định 10 giao dịch
    const parsed = Number(matched[1]);
    return Number.isFinite(parsed) && parsed >= 1 ? Math.min(parsed, 100) : 10;
  }

  // Trích xuất số ngày từ query (hỗ trợ: ngày, tuần, tháng)
  private extractDaysWindow(normalizedQuery: string): number {
    ...
    if (dayMatched?.[1]) {
      ...
      return Number.isFinite(parsed) && parsed >= 1
        ? Math.min(parsed, 365) // Tối đa 365 ngày
        : 30;
    }
  ...
  // Phát hiện ý định hỏi về lô sắp hết hạn
  private detectExpiringIntent(normalizedQuery: string): boolean {
    const keywords = [
      "sap het han", "duoi 1 thang", "con han", "het han trong",
      "can han", "can date", "gan het han", "near expiry",
      "near-expiry", "expiring", "sap qua han", "toi han", "han dung",
    ];

    return this.containsAny(normalizedQuery, keywords);
  }

  // Phát hiện ý định hỏi về lô đã hết hạn
  private detectExpiredIntent(
    normalizedQuery: string,
    asksExpiringSoon: boolean,
  ): boolean {
    const explicitExpiredKeywords = [
      "da het han", "qua han", "expired", "het date",
      "qua date", "expire roi", "het hsd",
    ];

    // Nếu có từ khóa explicit về expired
    if (this.containsAny(normalizedQuery, explicitExpiredKeywords)) {
      return true;
    }

    // "het han" có thể là sắp hết hoặc đã hết - kiểm tra context
    return normalizedQuery.includes("het han") && !asksExpiringSoon;
  }

  // Chuẩn hóa văn bản để so khớp (bỏ dấu, lowercase)
  ...
  // Trích xuất lô gần hết hạn nhất từ citations (kết quả RAG)
  private extractNearestExpiryLotFromCitations(
    ...
    // Duyệt qua từng citation để tìm ngày hết hạn
    for (const citation of citations || []) {
      ...
      // Bỏ qua nếu không parse được hoặc đã hết hạn
  ...
  // Trích xuất tóm tắt giao dịch từ dữ liệu thô
  ...
  // Định dạng ngày giờ sang định dạng Việt Nam
