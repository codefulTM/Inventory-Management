/**
 * OperatorProductionBatchDetail Page
 * Trang xem chi tiết Production Batch (Lô sản xuất) dành cho Operator
 * 
 * Chức năng chính:
 * - Hiển thị thông tin chi tiết lô sản xuất: mã lô, sản phẩm, kích thước, ngày SX, hạn dùng
 * - Quản lý nguyên liệu (Batch Components): xem, thêm, xóa, ghi số lượng thực tế
 * - Thay đổi trạng thái lô: In Progress → Complete / On Hold / Resume
 * - Xóa lô (chỉ khi không ở trạng thái In Progress)
 * 
 * Quy trình sản xuất:
 * 1. Tạo lô mới (status: "On Hold")
 * 2. Thêm nguyên liệu cần thiết cho lô (Batch Components)
 * 3. Chuyển trạng thái sang "In Progress" khi bắt đầu sản xuất
 * 4. Trong quá trình SX, ghi nhận số lượng thực tế (Actual Qty) cho từng nguyên liệu
 * 5. Hoàn thành: Chuyển sang "Complete" khi kết thúc
 * 
 * Lưu ý: 
 * - Chỉ có thể thêm/xóa nguyên liệu khi lô ở trạng thái "On Hold"
 * - Chỉ có thể ghi Actual Qty khi lô đang "In Progress"
 */
...
const STATUS_COLORS: Record<string, string> = {
  'In Progress': 'bg-blue-100 text-blue-700',  // Đang sản xuất
...
// ─── Modal: Thêm nguyên liệu cho lô (Add Component) ──────────────────────────
// Cho phép thêm một lô nguyên liệu (Inventory Lot) vào lô sản xuất
function AddComponentModal({
  batchId,
  onClose,
  onSaved,
}: {
  batchId: string;  // Mã lô sản xuất
  onClose: () => void;  // Đóng modal
  onSaved: () => void;  // Callback khi thêm thành công
}) {
  const [form, setForm] = useState({
    lot_id: '',  // Mã lô nguyên liệu (phải tồn tại trong kho và đã QC Accepted)
    planned_quantity: '',  // Số lượng kế hoạch
    unit_of_measure: '',  // Đơn vị tính
    added_by: '',  // Người thêm (tùy chọn)
    addition_date: new Date().toISOString().slice(0, 10),  // Ngày thêm
  });
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState('');

  // Xử lý submit thêm nguyên liệu
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createBatchComponent(batchId, {
        component_id: "",  // Tự động sinh bởi backend
        batch_id: batchId,
        lot_id: form.lot_id,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-black text-gray-900">Thêm Nguyên Liệu (Batch Component)</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="text-[10px] text-gray-400 mt-1">
              Nhập UUID của lô nguyên liệu đã được QC Accepted
            </p>
          </div>
...
// ─── Modal: Ghi số lượng thực tế (Record Actual Quantity) ───────────────────
// Cho phép ghi nhận số lượng nguyên liệu thực tế đã sử dụng
function RecordActualModal({
  ...
  batchId: string;
  component: BatchComponent;  // Nguyên liệu cần ghi số lượng
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateBatchComponent(batchId, component.component_id || "", {
        actual_quantity: actualQty as any,  // Cập nhật số lượng thực tế
      });
      onSaved();  // Reload dữ liệu
      onClose();  // Đóng modal
    } catch (e: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-black text-gray-900">Ghi Số Lượng Thực Tế</h3>
          <button onClick={onClose} className="p-1 hover:bg-gray-100 rounded-lg">
            <X size={20} />
          </button>
        </div>
        <p className="text-xs text-gray-400 mb-4">
          Lô: <span className="font-mono font-bold text-gray-600">{component.lot_id.substring(0, 16)}...</span>
          <br />
          Kế hoạch: <span className="font-bold text-gray-700">{component.planned_quantity} {component.unit_of_measure}</span>
        </p>
        {err && (
          <div className="mb-4 p-3 bg-red-50 text-red-700 rounded-xl text-sm font-bold">
            {err}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {saving ? 'Đang Lưu...' : 'Lưu SL Thực Tế'}
            </button>
          </div>
        </form>
      </div>
...
// ─── Trang chính: Chi tiết Production Batch ─────────────────────────────────
export default function OperatorProductionBatchDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  
  // State lưu thông tin lô sản xuất
  const [batch, setBatch] = useState<ProductionBatch | null>(null);
  // State lưu danh sách nguyên liệu của lô
  const [components, setComponents] = useState<BatchComponent[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  // State điều khiển các modal
  const [showAddModal, setShowAddModal] = useState(false);  // Modal thêm nguyên liệu
  const [recordingComp, setRecordingComp] = useState<BatchComponent | null>(null);  // Modal ghi SL thực tế
  const [deletingCompId, setDeletingCompId] = useState<string | null>(null);  // Đang xóa component nào
  const [updatingStatus, setUpdatingStatus] = useState(false);  // Đang cập nhật trạng thái

  // Hàm tải dữ liệu lô và nguyên liệu
  const load = useCallback(async () => {
    if (!id) return;
    setLoading(true);
    setError(null);
    try {
      // Tải song song thông tin lô và danh sách nguyên liệu
  ...
  const handleDeleteBatch = async () => {
    ...
    if (!window.confirm(`Xóa lô "${batch.batch_number}"?`)) return;
    try {
  ...
  const canComplete = batch.status === 'In Progress';  // Có thể hoàn thành
  const canHold = batch.status === 'In Progress';  // Có thể tạm dừng
  const canResume = batch.status === 'On Hold';  // Có thể tiếp tục
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header: Nút quay lại và các hành động */}
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {canHold && (
            ...
            >
              <PauseCircle size={15} /> Tạm Dừng (On Hold)
            </button>
          )}
          {/* Nút tiếp tục */}
          {canResume && (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {[
            { label: 'Mã Sản Phẩm', value: batch.product_id },
            {
              label: 'Kích Thước Lô',
              value: `${batch.batch_size} ${batch.unit_of_measure}`,
            },
            { label: 'Đơn Vị', value: batch.unit_of_measure },
            {
              label: 'Ngày Sản Xuất',
      ...
      {/* Danh sách nguyên liệu (Batch Components) */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Package size={18} className="text-blue-600" />
            <span className="font-black text-gray-700">
              Nguyên Liệu Sử Dụng ({components.length})
            </span>
          </div>
          {/* Chỉ cho phép thêm nguyên liệu khi lô chưa hoàn thành/hủy */}
        ...
        {components.length === 0 ? (
          <div className="p-12 text-center text-gray-300">
            ...
            <p className="text-sm font-bold">Chưa có nguyên liệu nào</p>
            <p className="text-xs mt-1 text-gray-300">
              Thêm lô nguyên liệu cần dùng cho lô này
            </p>
          </div>
        ) : (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-1">
                          {/* Nút ghi số lượng thực tế */}
                          <button
                            title="Ghi SL Thực Tế"
