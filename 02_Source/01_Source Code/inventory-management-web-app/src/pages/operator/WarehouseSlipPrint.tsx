/**
 * WarehouseSlipPrint Page (Operator)
 * Trang xem trước và in phiếu nhập/xuất kho
 * 
 * Chức năng chính:
 * - Tạo bản xem trước (preview) phiếu kho dạng HTML
 * - Hỗ trợ in phiếu trực tiếp từ trình duyệt
 * - Hiển thị thông tin: mã phiếu, loại phiếu, ngày tạo, danh sách vật tư
 * - Hiển thị tệp đính kèm (attachments) nếu có
 * 
 * Kỹ thuật in:
 * - Sử dụng iframe ẩn để tránh bị chặn popup
 * - Tự động kích hoạt lệnh in của trình duyệt
 * - Định dạng A4, có CSS riêng cho bản in (@media print)
 */
...
export default function WarehouseSlipPrint() {
  const { id } = useParams();
  // State lưu HTML đầy đủ để in (có thẻ <html>, <head>)
  const [html, setHtml] = useState<string>("");
  
  // previewHtml is the fragment we render inside the app for preview (no full <html> wrapper)
  // State lưu HTML xem trước (không có thẻ <html> bao ngoài)
  const [previewHtml, setPreviewHtml] = useState<string>("");

  // Tải dữ liệu phiếu và tạo HTML khi có ID
  useEffect(() => {
    if (!id) return;
    fetchWarehouseSlip(id).then((slip) => {
      // Tạo các dòng chi tiết phiếu (lines)
      ...
      // Tạo HTML đầy đủ để in (có khai báo DOCTYPE, html, head, body)
      ...
      // Lưu HTML xem trước (không có thẻ bao ngoài)
      setPreviewHtml(style + bodyHtml);
    });
  }, [id]);

  // Kiểm tra thiếu ID
  if (!id) return <div className="p-4">Thiếu mã phiếu</div>;

  // Hàm xử lý in phiếu
  function handlePrint() {
    // Use an offscreen iframe to avoid popup blockers and ensure printing works.
    // Sử dụng iframe ẩn để tránh bị chặn popup và đảm bảo in được
    try {
      ...
      // Chờ render xong, sau đó in và xóa iframe
      setTimeout(() => {
        try {
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold">Xem Trước Phiếu In</h1>
