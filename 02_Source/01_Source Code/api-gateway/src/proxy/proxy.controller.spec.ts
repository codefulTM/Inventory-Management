...
const makeFetchResponse = (status: number, body: string, contentType = 'application/json') => ({
  status,
...
describe('ProxyController', () => {
  ...
  describe('upstream errors', () => {
    it('throws HttpException 502 when fetch fails (network error)', async () => {
      ...
      await expect(controller.proxy(req as any, res as any)).rejects.toThrow(HttpException);
    });
