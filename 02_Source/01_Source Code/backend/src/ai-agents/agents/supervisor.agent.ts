...
export class SupervisorAgent {
  ...
  private deriveIntentFromDomainHints(
    ...
    const combined = `${normalizedAction} ${normalizedQuery}`.trim();
    ...
    if (warehouseHints.some((hint) => combined.includes(hint))) {
      return {
    ...
    if (inventoryHints.some((hint) => combined.includes(hint))) {
      return {
    ...
    if (qcHints.some((hint) => combined.includes(hint))) {
      return {
  ...
  private inferActionIfMissing(
    ...
    if (intent === AgentIntent.WAREHOUSE_OPERATOR) {
      if (
        ...
        normalized.includes('tao lot') ||
        normalized.includes('tạo lot') ||
    ...
    if (
      ...
      (normalized.includes('submit') ||
        ...
        normalized.includes('ket qua qc') ||
        normalized.includes('kết quả qc'))
    ) {
