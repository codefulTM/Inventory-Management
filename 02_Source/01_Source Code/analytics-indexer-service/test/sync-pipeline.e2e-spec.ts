...
async function buildModule(overrides: {
  ...
  const watermarkGetFn = overrides.watermarkGet ?? jest.fn().mockResolvedValue(null);
  const watermarkSetFn = overrides.watermarkSet ?? jest.fn().mockResolvedValue(undefined);

  const esBulkFn = overrides.esBulk ?? jest.fn().mockResolvedValue(makeBulkResponse(0));
  const esDeleteFn = overrides.esDelete ?? jest.fn().mockResolvedValue(makeDeleteResponse(0));
  ...
  const testModule: TestingModule = await Test.createTestingModule({
    providers: [
      ...
      { provide: getModelToken(InventoryTransaction.name), useValue: transactionsModel },
      { provide: getModelToken(QCTest.name), useValue: qcModel },
      { provide: getModelToken(Material.name), useValue: materialsModel },
      { provide: getModelToken(InventoryAuditReport.name), useValue: auditModel },
...
describe('Sync pipeline — full cycle (E2E)', () => {
  ...
  describe('Live documents — inventory_lots collection', () => {
    ...
    it('bulk-indexes 1 live lot document into the monthly index', async () => {
      ...
      expect(bulkOps[0]).toEqual(
        expect.objectContaining({ index: expect.objectContaining({ _index: 'inventory_lots_2026_04' }) }),
      );
    });
    ...
    it('watermark is updated to the cycle timestamp after successful index', async () => {
      ...
      const lotsCalls = (watermarkSet as jest.Mock).mock.calls.filter((c) => c[0] === 'inventory_lots');
  ...
  describe('Soft-deleted documents', () => {
    it('routes doc with deleted=true to bulkDelete, not bulkIndex', async () => {
      ...
      expect(bulkOps[0]).toEqual(
        expect.objectContaining({ delete: expect.objectContaining({ _id: 'lot-deleted-001' }) }),
      );
    });
  ...
  describe('Multi-collection sync — all 6 collections have data', () => {
    ...
    it('uses correct index names for each collection', async () => {
      ...
      const makeModel = (name: string) => {
        const doc = { _id: { toString: () => `${name}-001` }, modified_date: date, created_date: date };
        const m = makeModelMock([]);
        m.exec.mockResolvedValueOnce([doc]).mockResolvedValueOnce([]);
        return m;
      };
  ...
  describe('Failure isolation — one collection failing', () => {
    ...
    it('continues syncing remaining collections after one fails', async () => {
      ...
      const doc = { _id: { toString: () => 'mat-001' }, modified_date: new Date(), created_date: new Date() };
      materialsModel.exec.mockResolvedValueOnce([doc]).mockResolvedValueOnce([]);
  ...
  describe('SyncScheduler — concurrency guard', () => {
    it('invokes runFullSync on each tick', async () => {
      ...
      const spy = jest.spyOn(syncService, 'runFullSync').mockResolvedValue(undefined);
    ...
    it('skips tick when previous cycle is still running', async () => {
      ...
      const hangingPromise = new Promise<void>((res) => { resolveSync = res; });
      const spy = jest.spyOn(syncService, 'runFullSync').mockReturnValue(hangingPromise);
    ...
    it('resets running flag after sync completes', async () => {
      const { syncScheduler, syncService } = await buildModule({});
      jest.spyOn(syncService, 'runFullSync').mockResolvedValue(undefined);
    ...
    it('resets running flag even when sync throws', async () => {
      const { syncScheduler, syncService } = await buildModule({});
      jest.spyOn(syncService, 'runFullSync').mockRejectedValue(new Error('crash'));
